...
class MultiMenu extends Component {
  constructor(props) {
    super(props)
    console.log('参数：', this.props.confData)
            ...
            <ul className="rob-hormenu-ul2-basic">
              {obj.ChildMenu ? obj.ChildMenu.map((childObj, k) => (
                <li key={k} className="rob-hormenu-li2" onClick={(event) => this.handleClick(event, childObj)}>
                  <span><i className={`${childObj.iconClass} rob-hormenu-icon-left`} />&nbsp;&nbsp;&nbsp;&nbsp;{childObj.name}{childObj.grandchild.length > 0 ? <i className="rob-hormenu-icon-right rob-icon-angle-up" /> : null}</span>
