...
class FileUpload extends PureComponent {
  ...
  _updateProps(props) {
    ...
    if (this.chooseAndUpload) {
      React.Children.forEach(props.children, (child) => {
        const { props = {} } = child // eslint-disable-line
        if (child && child.type === 'chooseAndUpload') {
          chooseBtn = React.cloneElement(child, { ref: r => this._chooseAndUpload = r })
          flag++
        } else {
      ...
      React.Children.forEach(props.children, (child) => {
        const { props = {} } = child // eslint-disable-line
        if (child && props.type === 'chooseBtn') {
          chooseBtn = React.cloneElement(child, { ref: r => this._chooseBtn = r })
          flag++
        } else if (child && props.type === 'uploadBtn') {
          uploadBtn = React.cloneElement(child, { ref: r => this._uploadBtn = r })
          flag++
        } else {
  ...
  commonChooseFile = () => {
    ...
    this.ajax_upload_file_input.click()
  }
  ...
  commonUpload = () => {
    ...
    if (jud !== true && jud !== undefined && typeof jud !== 'object') {
      /* 清除input的值*/
      this.ajax_upload_file_input.value = ''
      return
    }
    ...
    xhr.onprogress = xhr.upload.onprogress = progress => { // eslint-disable-line
  ...
  IEUpload = (e) => {
    ...
    if (this.__params) {
      ...
      for (const key in this.__params) {
        paramArr.push(`${key}=${this.__params[key]}`)
      }
      let __params = paramStr ? `&${paramArr.join('&')}` : `?${paramArr.join('&')}`
      paramStr += __params
    }
  ...
  IECallback(dataType, frameId) {
    ...
    try {
      ...
      resp.json = JSON ? JSON.parse(resp.responseText) : eval(`(${resp.responseText})`) // eslint-disable-line
    } catch (e) {
      /* 如果是包含了<pre>*/
      if (e.message && e.message.indexOf('Unexpected token') >= 0) {
        ...
        if (resp.responseText.indexOf('{') >= 0) {
          ...
          return JSON ? JSON.parse(msg) : eval(`(${msg})`) // eslint-disable-line
        }
      ...
      render = (
        <div className={this.props.className} style={this.props.style}>
          ...
          <input
            ...
            ref={r => this.ajax_upload_file_input = r}
            style={{ display: 'none' }}
  ...
  _multiIEForm = () => {
    ...
    function _insertIEForm(formArr, i) { // eslint-disable-line
      ...
      const input = (
        <input
          ...
          ref={r => this[`ajax_upload_hidden_input_${i}`] = r}
      ...
      i = `${this.IETag}${i}` // eslint-disable-line
      formArr.push((
        <form
